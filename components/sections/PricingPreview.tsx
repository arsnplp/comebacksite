    ...
    <section className="py-20 sm:py-28" aria-labelledby="pricing-title">
      <Container>
        <SectionHeader
          ...
          lead="30 jours gratuits sur tous les plans, sans carte bancaire. Ensuite :"
        />
        ...
        <Stagger className="grid items-stretch gap-6 lg:grid-cols-3">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative flex h-full flex-col rounded-3xl bg-white p-8 shadow-card transition-all duration-300 hover:-translate-y-1.5 hover:shadow-card-lg ${
